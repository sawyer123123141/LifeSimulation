...
`AGENTS.md` is the contract for **implementation subagents** â narrow scope, do
...
saved time had you known it at the start, add it to Â§5 with a date. When the
live/dead status of a mechanism changes, update Â§4 in the same commit as the
code. Keep entries short and concrete â a rule nobody reads is worthless.

---

## 1. Where things are

Grep by **name**, not by line number â line numbers drift and this file will lie
to you within a week. Names below are stable.

### Simulation core â `Assets/Scripts/Simulation/`
...
| `Biology/GenomeInheritance.cs` | Crossover and mutation | `CreateChild`, `InheritTrait` (trait indices are positional â see Â§5) |
...
| `Environment/PlantSiteRegistry.cs` | Which sites are free to colonize | â |
...
| `Diagnostics/LivenessRecorder.cs` | Runtime probe counters for code *paths*; covers the §4 "runs on empty data" class perturbation cannot reach. Attach via `SimulationWorld.Liveness` (null by default); never touches any hash. | `LivenessProbe`, `RecordOutcome`, `IsInertlyExecuting` |

### Elsewhere

- `Assets/Scripts/Presentation/Prototype1Presenter.cs` â Unity view, playtest
  hotkeys (`N`, `E`), the creature inspector. `_world` is non-serialized;
  `EnsureInitialized()` guards domain reloads during Play mode.
- `Assets/Editor/PrototypeBatchEntry.cs` â batch experiment entry points.
- `Assets/Tests/EditMode/*.cs` â 368 tests. `ResourceExperimentTests.cs` holds
  the scenario/calibration tests; `CoreSimulationTests.cs` is the big one;
  `LivenessTests.cs` enforces the Â§4 gene ledger by perturbation.
- `tools/HeadlessTests/` â `dotnet test` runs the EditMode tests headlessly.
  This is how you run tests; Unity is only needed for Play mode.

### Documentation worth knowing exists

- `docs/superpowers/specs/2026-08-14-simulation-defects-and-behavior-gaps.md` â
  **the known-defect register (B-1, B-4, B-7 â¦).** Check here before "fixing" a
  bug; it records which defects are deliberate deferrals.
- `docs/experiments/` â dated experiment records. Several supersede each other;
...
4. **Is it actually reachable?** A defect in code that never executes is not
   ...
   mechanisms in this repo are written, tested, and never reached (Â§4). Two of
   those were deleted on 2026-08-18; check Â§4, do not assume the count.
...
reaches production behavior.** See the `Persistence` entry in Â§5.
...
- 30-seed Ã 12,000-tick sweeps take a few minutes. Run them in the background
  and do something else. Five seeds is usually not enough to distinguish arms â
...
scenario may simply have no occasion to fire â `RiskAversion` reads dead under
...
| `Genome.NeutralMarker` | Renamed from `Genome.Commitment` on 2026-08-18: the old name collided with the unrelated `ForagingEconomics.CommitmentBonus` and `SimulationConfig.CommitmentStrength` foraging machinery, which takes `Persistence`. That collision is exactly what led the 2026-08-17 audit to assume the gene fed the bonus. Inherited, mutated, hashed, aggregated into statistics, exposed as `ExperimentMetric.NeutralMarker` â and read by **zero** behavior code. **Kept deliberately** as the drift-control channel that validates the bootstrap pipeline; confirmed dead by perturbation under FULL ecosystem mode and pinned by `LivenessTests`. Do not wire it. |

**Executes but always on empty data:** `TryScoreBestRememberedPlace`,
`RecordFailedPlaceSearch` â both depend on place-memory slots, which are never
populated.

> **Now enforced.** Neither perturbation harness can reach this class â there is
> no gene and no flag to flip for place memory â so it was documented and nothing
...
> production reader". That is true of all four above and **insufficient** â the
...
pins the set and fails the build if it changes â a flag becoming live is a real
...
**2026-08-17 â A mechanism story is not evidence.** Four conclusions were wrong
...
**2026-08-17 â Transcribe measured configurations; never reconstruct one that
...
**2026-08-17 â Confounded sweeps.** When a sweep generates its arms
...
**2026-08-17 â Positional constructor arguments silently drop genes.**
...
**2026-08-17 â A static caller-search is not a liveness check.** Grepping for
...
score** â and must be excluded from `ComputeStateHash` so it does not change the
simulation it measures.

**2026-08-17 â Engine bounds are not ecological parameters.** The calibration
...
**2026-08-17 â A failing regression guard is usually right.** The six-site
...
**2026-08-18 â When a trait will not move, check that it *can* move before
...
**2026-08-18 â A prediction that fails is still a measurement; report both.**
The prediction here was that defense would be selected *down*, being a cost that
buys nothing. It was not â deltas were pure drift. The cost was unrealized too,
...
**2026-08-18 â Check the sign of a feedback, not just its magnitude.** Defense
...
**2026-08-18 â A caller-search misses readers inside aggregate expressions.**
...
inside the `bodyMass` weighted sum â a real production path found immediately by
perturbation. Grep finds *names*; it does not tell you whether the value flows
anywhere. Use `LivenessTests`.

**2026-08-18 â A trait that only moves while the ecosystem collapses has not
demonstrated a gradient.** Sweeping plant regeneration down finally moved defense
(+0.1157 at founder 0.6, regeneration 3) â in an arm where 30/30 seeds lost their
...
**2026-08-18 â A one-dimensional pressure lever moves two things here.**
...
grazing bites locally without starving the population globally â the arms trade
...
**2026-08-18 â Selection cannot act without standing variance, and a uniform
...
**2026-08-18 â A control that moves the trait downward is still a valid positive
...
**2026-08-18 â A clamped score term silently deletes a whole decision dimension.**
...
quality was therefore invisible to `IntentUtilityV1` foraging â grazers chose on
...
**2026-08-18 â Identical results across an arm flip means the flag is dead, not
...
every cell. That is not a weak effect â floating-point ecology does not reproduce
...
**2026-08-18 â Per-patch drawdown and population survival cannot be separated by
...
biomass than the system regrows â starvation by definition. Selection on a defense
...
**2026-08-18 â Perturbation is blind to code that runs on empty data.** Flipping a
...
thing that catches it — pair it with one probe on a known-live path, or a silently
broken recorder looks identical to a correct one reporting all-inert.

**2026-08-18 â Perturbation must cover config flags, not just genes.** Four of
...
**2026-08-18 â A name collision is a defect when it makes a wrong belief
plausible.** `Genome.Commitment` sat beside `ForagingEconomics.CommitmentBonus`
and `SimulationConfig.CommitmentStrength`, all unrelated â the bonus takes
...
**2026-08-18 â Liveness verdicts are scoped to the scenario, and a narrow
...
- Two decision policies exist: `Legacy` and `IntentUtilityV1`. **Every P4
  ...
  is the source of most half-wired mechanisms â check both paths.
...
- `PlantDefenseDeterrenceEnabled` defaults `false`. With it off, plant defense
  protects no biomass and cannot be selected on at all â see
...
- Phase order is fixed: P4 (ecosystem) â P5 (species/history) â P6 (terrain
