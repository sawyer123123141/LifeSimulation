...
**CLOSED 2026-08-29, and the claim it made was wrong.** This block said no `.unity` scene was
committed and concluded the visual layer existed only in one machine's Unity state. **It does not.**
`Prototype1Presenter` carries a `[RuntimeInitializeOnLoadMethod(AfterSceneLoad)]` that creates itself
when no instance is present, then builds its camera, both lights, terrain, water and creature views in
code. **Any scene works, including an empty one, and there is nothing in a scene file to lose.** The
claim was made from `git ls-files '*.unity'` and `m_Scenes: []` without ever reading how the presenter
starts, and it was then repeated for hours as the project's biggest risk.

What was really wrong is much smaller and is now fixed: `m_Scenes: []` meant a **built player** had no
scene registered and would have started on nothing. `Assets/Scenes/Prototype1.unity` is generated by
`SceneBootstrap` (menu: Life Simulation), deliberately empty, and registered as the only build scene.
Verified by building a real player in batch mode: `BUILD result=Succeeded errors=0`, a 95 MB
`LifeSim.exe`. In the editor this never showed, because pressing Play on any open scene is enough for
the bootstrap to fire.
...
**The scene claim attached to this finding was wrong and is corrected above** - the presenter
self-instantiates, so no scene content was ever at risk. The `.meta` work stands on its own reasons:
GUIDs, and the FBX `ModelImporter` settings.
...
zero tracked `.unity` files outside `Assets/_Recovery/`). **The conclusion drawn from that - that the
scene wiring lived only in someone's local Unity state - was wrong.** `Prototype1Presenter` creates
itself via `[RuntimeInitializeOnLoadMethod]` and builds everything in code, so Play mode on an empty
Hierarchy is the normal case and produces the whole world. The real consequence was that a built
player had no scene; fixed by `SceneBootstrap`, verified with an actual player build.
