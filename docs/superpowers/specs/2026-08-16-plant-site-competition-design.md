...
- **Resolved (follow-up):** final review found `FindSite` only drew
  candidates from `PlantSiteRegistry`, which registered initially-*inactive*
  dispersal-target sites only — founder patches (e.g. the ones under defense
  selection in `ConsumerDefenseCalibrationControl`/`Moderate`) could never be
  taken over. `SimulationScenario.ApplyTo` now also registers initially-
  *active* Food sites into `PlantSites`, but only when
  `PlantSiteCompetitionEnabled` is `true` — so `PlantSiteRegistry.Count`,
  and therefore every existing dispersal-target RNG draw, is completely
  unchanged for any scenario that leaves the flag at its default `false`.
